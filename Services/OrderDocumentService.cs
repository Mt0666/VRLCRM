...
public class OrderDocumentService
{
    ...
    public byte[] GenerateExcel(Order order)
    {
        ...
        sheet.Range("A1:G1").Merge().Value = $"{_company.CompanyName} - Sipariş Formu";
        sheet.Range("A1:G1").Style.Font.SetBold().Font.SetFontSize(16).Fill.SetBackgroundColor(XLColor.FromHtml(BrandColor)).Font.SetFontColor(XLColor.White);
        ...
        {
            var productText = line.StockItem.Name;
            if (!string.IsNullOrWhiteSpace(line.Notes))
            {
                productText += " — " + line.Notes;
            }
            ...
            if (row % 2 == 0)
            {
                sheet.Range(row, 1, row, 7).Style.Fill.SetBackgroundColor(XLColor.FromHtml("#F8F8FB"));
            }
            row++;
        }

        row++;
        sheet.Range(row, 5, row, 6).Merge().Value = "Mal Hizmet Toplam Tutarı";
        sheet.Cell(row, 5).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Right);
        sheet.Cell(row, 7).Value = order.SubTotal;
        sheet.Cell(row, 7).Style.NumberFormat.SetFormat("#,##0.00 \"₺\"");
        row++;
        sheet.Range(row, 5, row, 6).Merge().Value = "Hesaplanan KDV";
        sheet.Cell(row, 5).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Right);
        ...
        if (order.DiscountRate > 0)
        {
            ...
            sheet.Cell(row, 5).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Right);
            sheet.Cell(row, 7).Value = -order.DiscountAmount;
            sheet.Cell(row, 7).Style.NumberFormat.SetFormat("#,##0.00 \"₺\"");
        }
        row++;
        sheet.Range(row, 5, row, 6).Merge().Value = "Ödenecek Tutar";
        ...
        if (!string.IsNullOrWhiteSpace(order.Notes))
        {
            ...
            sheet.Range(row + 1, 1, row + 1, 7).Merge().Value = order.Notes;
        }
